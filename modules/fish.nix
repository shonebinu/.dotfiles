{
  ...
  programs.fish = {
    ...
    interactiveShellInit = ''
      ...
      function jo
          set file "$HOME/journal/$(date +"%Y/%m/%d").md"

          if not test -f $file
              mkdir -p "$HOME/journal/$(date +'%Y/%m')"
              echo "# $(date +"%a %d-%m-%y")" > $file

              echo "" >> $file
              echo "prompt: What did you do today? How are you feeling?" >> $file
              echo "" >> $file
              echo "## Morning Journal Entry" >> $file
              echo "" >> $file
              echo "## Evening Journal Entry" >> $file
              echo "" >> $file
              echo "## Gratitude Journal Entry" >> $file
              echo "" >> $file
            hx $file
          else
              hx $file
